...
\begin{figure}[!ht]
	\placefigure{dynamics-2/ch-magnitude-increase}
	\placecaption{Average clonal haematopoiesis clone size stratified by age (Figure 1b in \cite{Fabre2021-uw}).}
	\label{fig:ch-magnitude-increase}
\end{figure}

\begin{figure}[!ht]
	\placefigure{dynamics-2/ch-prevalence-increase}
	\placecaption{Clonal haematopoiesis prevalence and clonality stratified by age (Figure 1c in \cite{Fabre2021-uw}).}
	\label{fig:ch-prevalence-increase}
\end{figure}

\begin{figure}[!ht]
	\placefigure{dynamics-2/ch-prevalence-genes}
	\placecaption{Prevalence of mutations in different genes in clonal haematopoiesis (top). Age-stratified prevalence of mutations \textit{DNMT3A}, \textit{TET2} and spliceosome (\textit{SF3B1}, \textit{SRSF2} and \textit{U2AF1}) genes (bottom) (Figure 1d in \cite{Fabre2021-uw}).}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/ch-mutations-per-gene}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/ch-trajectories-explained-mut-tp}
...
\begin{figure}[!ht]
    \placefigure{dynamics-2/ch-unexplained-trunc-site-sex-smoke}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/ch-bad-trajectories-examples}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/trajectories-6th-tp}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/performance-6th-tp-2}
	\placecaption{Observed and predicted variant allele frequency (VAF) for a 6th timepoint (based on Extended Data Figure 3f in \cite{Fabre2021-uw}).}
	\label{fig:performance-6th-tp-2}
\end{figure}

\begin{figure}[!ht]
	\placefigure{dynamics-2/performance-6th-tp}
	\placecaption{Observed and predicted fold change in variant allele frequency (VAF) for a 6th timepoint (Extended Data Figure 3f in \cite{Fabre2021-uw}).}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/ch-gene-site-coefficients}
	\placecaption{Driver growth per year for different genes and sites. Individual points and vertical lines represent the expected value and 90\% highest posterior density interval for sites and horizontal lines and rectangles represent the expected value and 90\% highest posterior density interval for genes (Figure 2c in \cite{Fabre2021-uw}).}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/ch-trunc}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/ch-mean-std-growth}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/ch-variance-uc-technical}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/ch-variance-uc-biological}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/trees-pd42176}
	\placecaption{Ultrametric phylogenetic tree for individual PD42176, with all studied driver mutations annotated and relevant structural rearrangements (left). Lifelong trajectory for the \textit{SF3B1}-K666N, inferred using both longitudinal targeted sequencing data and phylodynamic methods (Figure 3a,d in \cite{Fabre2021-uw}).}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/trees-pd34493}
	\placecaption{Ultrametric phylogenetic tree for individual PD34493, with all studied driver mutations annotated and relevant structural rearrangements (left). Lifelong trajectory for the \textit{SF3B1}-K666N and \textit{U2AF1}-Q157R, inferred using both longitudinal targeted sequencing data and phylodynamic methods (right) (Figure 3b,e,f in \cite{Fabre2021-uw}).}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/trees-pd41305}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/trees-coefficients-ages-at-onset}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/exceeds-within-lifetime-representation}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/trajectories-fabre-mitchell}
	\placecaption{Lifelong clonal trajectories inferred for \cite{Fabre2021-uw} (left) and \cite{Mitchell2021-zl} (right). Fractions of the trajectories showing low variance ($V(t) < 5$) are represented as full lines, whereas fractions with high variance ($V(t) \geq 5$) are represented as dotted lines. A thin grey band represents the carrying capacity from \cite{Lee-Six2018-lp} (50,000-200,000 haematopoietic stem cells) (Figure 4a,b in \cite{Fabre2021-uw}).}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/expected-observed-ratio-representation}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/expected-observed-ratio}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/expected-observed-full}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/exceeds-within-lifetime}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/fraction-clones-exceeding-lifetime}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/historical-growth-representation}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/deceleration-figure-full}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/historical-growth-tally}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/ages-at-onset}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/ages-at-onset-benchmark}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/smoking-age-density}
...
\begin{figure}[!ht]
	\placefigure{dynamics-2/ch-risk-selection}
	\placecaption{Association between annual growth and acute myeloid leukaemia (AML) risk (left) and selection in myelodysplastic syndrome (MDS) and AML (right) (based on Figure 5 in \cite{Fabre2021-uw}).}
